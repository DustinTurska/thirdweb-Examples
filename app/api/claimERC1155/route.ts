...
export async function POST(req: NextRequest) {
  try {
    ...
    const tokenId = body.tokenId || body.tokenId;
    ...
    console.log(`Attempting to claim for receiver: ${receiver}, tokenId: ${tokenId}, quantity: ${quantity}`);
    ...
    console.error("Error claiming ERC1155 tokens", error);
    if (error instanceof Error) {
      return NextResponse.json(
        { error: "Error claiming ERC1155 tokens", details: error.message },
        { status: 500 }
      );
    } else {
      return NextResponse.json(
  ...
  while (attempts < maxAttempts) {
    try {
      ...
      if (status.result.status === "mined") {
        console.log(
          "Transaction mined! 🥳 ERC1155 token has been claimed",
          queueId
        );
